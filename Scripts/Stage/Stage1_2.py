...
class Stage1_2(MainStage):
    ...
    def Init(self):
        ...
        tile.MakeTile(62, 30 - self.height)
        self.tiles.append(tile)

        tile = MakeTile_X(6, self.stage)
        tile.MakeTile(330, 30 - self.height)
        self.tiles.append(tile)

        tile = MakeTile_X(6, self.stage)
        tile.MakeTile(580, 30 - self.height)
        self.tiles.append(tile)

        # 1층 왼쪽 타일
        tile = MakeTile_X(6, self.stage)
        tile.MakeTile(120, 133 - self.height)
        self.tiles.append(tile)

        # 1층 중앙 타일
        tile = MakeTile_X(3, self.stage)
        tile.MakeTile(380, 133 - self.height)
        self.tiles.append(tile)

        # 1층 오른쪽 타일
        tile = MakeTile_X(6, self.stage)
        tile.MakeTile(525, 133 - self.height)
        self.tiles.append(tile)

        # 2층 왼쪽 타일
        tile = MakeTile_X(8, self.stage)
        tile.MakeTile(140, 240 - self.height)
        self.tiles.append(tile)

        # 2층 오른쪽 타일
        tile = MakeTile_X(8, self.stage)
        tile.MakeTile(450, 240 - self.height)
        self.tiles.append(tile)

        # 3층 왼쪽 타일
        tile = MakeTile_X(8, self.stage)
        tile.MakeTile(140, 345 - self.height)
        self.tiles.append(tile)

        # 3층 오른쪽 타일
        tile = MakeTile_X(8, self.stage)
        tile.MakeTile(450, 345 - self.height)
        self.tiles.append(tile)

        # 4층 왼쪽 타일
        tile = MakeTile_X(8, self.stage)
        tile.MakeTile(140, 450 - self.height)
        self.tiles.append(tile)

        # 4층 오른쪽 타일
        tile = MakeTile_X(8, self.stage)
        tile.MakeTile(450, 450 - self.height)
        self.tiles.append(tile)